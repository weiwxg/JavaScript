...
```  
> 注意：**Number()函数在解析Object对象时，先获取对象的valueOf()值，然后转换为对应数字值，如果该值被转换后为NaN，再调用对象的toString()方法，获取其值再转换； parseInt()和parseFloat方法也可以接收一个对象参数，但在进行转换时只会调用对象的toString()方法获取值，再进行转换。**
